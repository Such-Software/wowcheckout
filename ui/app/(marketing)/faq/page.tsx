...
export const metadata: Metadata = {
  title: "FAQ",
  description:
    "Frequently asked questions about WOW Checkout: non-custodial Wownero invoices with view-only detection.",
};
...
const faqItems: FaqItem[] = [
	  {
	    question: "Is WOW Checkout custodial?",
	    answer: (
	      <>
	        <p className="text-ink-soft">
	          No. Payments go directly from the customer to your Wownero wallet. WOW
  ...
  {
    question: "What wallet access does WOW Checkout require?",
    answer: (
      <>
        <p className="text-ink-soft">
		  ...
		  },
  {
    ...
    answer: (
	  ...
	  {
	    question: "What does WOW Checkout cost?",
	    answer: (
	      <>
	        <p className="text-ink-soft">
	          Currently, wowcheckout.com does not charge a service fee. You can also self-host the open source
	          stack if you prefer.
	        </p>
	        {donationsEnabled ? (
	          <p className="text-ink-soft">
	            If you find WOW Checkout useful, you can support development with{" "}
  ...
  {
    question: "Can WOW Checkout move funds, issue refunds, or reverse payments?",
    answer: (
      <>
        <p className="text-ink-soft">
          No. WOW Checkout does not sign transactions and cannot move funds on
  ...
  {
    question: "What happens if WOW Checkout is down when a customer pays?",
    answer: (
      <>
        <p className="text-ink-soft">
	  ...
	  {
	    question: "How does WOW Checkout differ from hosted services (e.g. NOWPayments)?",
	    answer: (
	      <>
	        <p className="text-ink-soft">
	          The main differences are scope, custody model, and data collection: WOW Checkout is Wownero-only,
	        ...
	        <ul className="mt-4 grid gap-2 text-sm text-ink-soft">
	          <li>
	            <span className="font-semibold text-ink">Wownero-only:</span> a focused
	  ...
	  {
	    question: "How does WOW Checkout differ from BTCPay Server?",
	    answer: (
	      <>
	        <p className="text-ink-soft">
	          BTCPay Server is a general-purpose, self-hosted payment server with a primary focus on Bitcoin
	          and optional integrations for other assets (including Wownero via a plugin). WOW Checkout is intentionally
	          focused on Wownero checkout with a view-only trust boundary and a minimal, deterministic core.
	        </p>
	        <p className="text-ink-soft">
	          Some hosted BTCPay deployments may not include the Wownero plugin by default. WOW Checkout provides a
	          small compatibility layer so teams can integrate without running a full BTCPay deployment.
	        </p>
        <ul className="mt-4 grid gap-2 text-sm text-ink-soft">
          <li>
            <span className="font-semibold text-ink">Wownero-first UX:</span>{" "}
            invoice and confirmation language is tailored to Wownero.
          </li>
          <li>
  ...
  {
    question: "Can I self-host WOW Checkout?",
    answer: (
      <>
        <p className="text-ink-soft">
          Yes. WOW Checkout is open source, and you can self-host it from the
          official{" "}
          <Link
      ...
      <section className="px-[6vw] pb-20 pt-10">
        <div className="grid gap-4">
	          ...
	          <p className="max-w-[46rem] text-[1.02rem] leading-relaxed text-ink-soft">
	            WOW Checkout is non-custodial checkout software for accepting Wownero
