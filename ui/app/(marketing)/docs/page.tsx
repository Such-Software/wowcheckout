  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        description: "List enabled payment methods (WOW-CHAIN only).",
      },
    ],
  },
  {
...
const endpointRequirements = [
  ...
  },
  {
    ...
    required: ["store_id (path)"],
    optional: [],
    notes: ["Returns WOW-CHAIN as the only enabled payment method."],
  },
  {
    method: "POST",
    path: "/api/v1/stores/{store_id}/invoices",
    ...
    <main className="px-[6vw] pb-20 pt-10 text-ink">
      <section className="grid max-w-[52rem] gap-4">
        ...
        <p className="text-[1.05rem] leading-relaxed text-ink-soft">
          wowcheckout.com generates invoices, detects Wownero payments using view-only access,
      ...
      <section className="mt-10 rounded-2xl border border-stroke bg-white/60 p-6 shadow-soft backdrop-blur">
        ...
        <p className="mt-2 text-ink-soft">
          ...
          value to WOW using the current rate and returns a non-binding quote in the
          response. The invoice is always stored in WOW.
        </p>
      ...
      <section className="mt-6 grid gap-6 sm:grid-cols-2">
        <div className="rounded-xl border border-stroke bg-white/60 p-6 shadow-soft backdrop-blur">
          ...
          <ul className="mt-3 list-disc space-y-2 pl-5 text-ink-soft">
            ...
            <li>Invoices are defined in WOW; fiat inputs are quote-only metadata</li>
          ...
          {endpointGroups.map((group, index) => {
            ...
            return (
              <Fragment key={group.title}>
                {needsDivider ? (
                  <div className="h-px w-full bg-wownero/70 lg:col-span-2" aria-hidden="true"></div>
                ) : null}
