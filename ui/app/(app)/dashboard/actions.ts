...
export async function createInvoiceAction(
  ...
  if (Number.isNaN(amount) || amount <= 0) {
    return {
      error: isFiat ? "Enter a valid fiat amount." : "Enter a valid WOW amount.",
      invoiceId: null,
      address: null,
      amount: null,
      recipientName: null,
  ...
  const apiKey = (await cookies()).get("wowcheckout_api_key")?.value;
  if (!apiKey) {
    return {
...
export async function resetApiKeyAction(
  ...
  cookieStore.set("wowcheckout_api_key", data.api_key, {
    httpOnly: true,
    sameSite: "lax",
...
export async function resetWebhookSecretAction(
  ...
  const cookieStore = await cookies();
  cookieStore.set("wowcheckout_webhook_secret", data.webhook_secret, {
    httpOnly: true,
    sameSite: "lax",
    secure: process.env.NODE_ENV === "production",
    path: "/",
...
export async function logoutAction() {
  const cookieStore = await cookies();
  cookieStore.set("wowcheckout_api_key", "", {
    httpOnly: true,
    sameSite: "lax",
    secure: process.env.NODE_ENV === "production",
    path: "/",
  ...
  cookieStore.set("wowcheckout_webhook_secret", "", {
    httpOnly: true,
    sameSite: "lax",
    secure: process.env.NODE_ENV === "production",
    path: "/",
